...
void run_my_script(char *name, char *args){
	char script[120];

	snprintf(script, sizeof(script), "/usr/sbin/%s", name);

	if(f_exists(script)) {
		if (args)
			logmessage("my script" ,"Running %s (args: %s)", script, args);
		else
			logmessage("my script" ,"Running %s", script);
		xstart(script, args);
	}
}
